...
const SharedConfiguration = () => {
  ...
  useEffect(() => {
    const loadConfiguration = async () => {
      try {
        await getConfigurationByLink(link);
      } catch (err) {
  ...
  if (!currentConfig) {
    return (
      <Container maxWidth="lg">
        <Box sx={{ mt: 4, mb: 4 }}>
          <Alert severity="info">Loading shared configuration...</Alert>
        </Box>
      </Container>
    );
  }
      ...
      <Box sx={{ mt: 4, mb: 4 }}>
        <Box sx={{ mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
            ...
            <Divider orientation="vertical" flexItem />
            <Chip size="small" label="Shared Configuration" color="primary" variant="outlined" />
          </Box>
        </Box>
